	...
	<head>
		...
		<script type="text/javascript">
			...
			function updateSubject(){
				...
				$.ajax({
					type : "POST",
					url : "${AppAction}/SubjectBase/update",
					data : $("#PublishForm").serialize(),
					success : function(res) {
						if (res) {
							window.location = "${AppBase}/SDCHOME/bbs/SubjectList.jsp";
						} else {
							$("#subjectRet").html("修改失败！");
						}
					},
					fail : function(res) {
						alert("系统错误?!");
		...
		<div class="row">
			...
			<div class="col-xs-12">
				...
				<form class="form-horizontal" role="form" id='PublishForm'
					...
					<input type="hidden" name="authorid" id="authorid" value="${subject.authorid}"/>
					<input type="hidden" name="subjectid" id="subjectid" value="${subject.subjectid}"/>
					<div class="form-group">
						<input type="text" id="title" name='title' placeholder="添加文章标题" class="col-xs-10 col-sm-8" value="${subject.title}"/>
					</div>
					<div class="form-group">
						<textarea name="content" id="content" style="width:100%;min-height:600px;visibility:hidden;">${subject.content}</textarea>
					</div>
					<div class="form-group">
						<input type="text" name="keywords" id="keywords" style="width:100%;" placeholder="输入关键字方便查找......" value="${subject.keywords}" />
					</div>
		...
		<script type="text/javascript">
			jQuery(function($) {
				...
				$("#submitBtn").on(ace.click_event, function() {
					bootbox.dialog({
						message: "<span class='bigger-110'>是否确认将信息的修改保存到服务器</span>",
						buttons: {
							"success" :
							 {
